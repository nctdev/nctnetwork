...
export function Footer({ brandName = "NCTNetwork", brandColor = "#f97316" }: FooterProps) {
        ...
        <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
          <div className="text-center md:text-left">
            <div className="text-2xl font-bold mb-2">
              <span style={{ color: brandColor }}>{brandName.slice(0, 3)}</span>
              <span className="text-slate-700 dark:text-slate-300">{brandName.slice(3)}</span>
            </div>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              © {currentYear} {brandName}. Crafting digital excellence.
            </p>
          </div>
