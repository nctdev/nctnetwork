        ...
        <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5, delay: 0.3 }} className="border-[3px] border-orange-500 p-4 md:p-6 bg-white dark:bg-gray-800 shadow-lg" style={{ borderRadius: '6px' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
            {projects.map((project, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="p-4 rounded-xl dark:bg-gray-700 bg-gray-50 shadow-lg border border-gray-200 dark:border-gray-600">
                {project.status === "completed" ? (
                  <>
                    ...
                    <p className="text-sm mb-4 dark:text-gray-300 text-gray-600">{project.description}</p>
                    <div className="flex flex-wrap gap-2 mb-3">
                      <span className="px-3 py-1 text-xs rounded-lg bg-orange-500 text-white font-medium">{project.technology}</span>
                    </div>
                    <p className="text-sm dark:text-gray-400 text-gray-500">Year: {project.year}</p>
                  </>
                ) : (
                  <>
                    <h3 className="text-xl font-bold mb-2 dark:text-white text-gray-900">{project.title}</h3>
                    ...
                    <p className="text-sm mb-4 dark:text-gray-300 text-gray-600">{project.description}</p>
                    <div className="flex flex-wrap gap-2 mb-3">
                      <span className="px-3 py-1 text-xs rounded-lg bg-orange-500 text-white font-medium">Coming Soon</span>
                    </div>
                  </>
                )}
              </motion.div>
